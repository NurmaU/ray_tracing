...
vec3 random_in_init_sphere() {
    while (true) {
        vec3 res = vec3::random(-1, 1);  // TODO what happens with this object
                                         // if not used, is it memory efficient?
        if (res.length_squared() < 1.0) return res;
    }
}
